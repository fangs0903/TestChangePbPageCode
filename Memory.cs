...
namespace TestChangePbPageCode
{
    internal class Memory
    {
        ...
        public static int i3FrameworkDx;
        public static int i3FrameworkDxSize;
        public static bool TryGetModule;
        public static bool TryGetProcess;
        ...
        public static bool OpenProcess(string name)
        {
            try
            {
                if(name != string.Empty)
                {
                    ...
                    WindowsName = Process.GetProcessesByName(name)[0].MainWindowTitle;
                    TryGetProcess = true;
                    return true;
                }
                return false;
        ...
        public static bool ProcessHandle()
        {
            ...
            {
                Debug.WriteLine("Can't get handle.");
                return false;
            }
        }
        ...
        public static bool GetModules()
        {
            try
            {
                ...
                {
                    if (module.ModuleName == "i3FrameworkDx.dll")
                    {
                        i3FrameworkDx = (int)module.BaseAddress;
                        i3FrameworkDxSize = (int)module.ModuleMemorySize;
                    }
                }
                ...
                if (/*(IntPtr)Client == IntPtr.Zero || (IntPtr)Engine == IntPtr.Zero || */(IntPtr)i3FrameworkDx == IntPtr.Zero)
                {
                    Debug.WriteLine("Module error");
                    TryGetModule = false;
                    return false;
                }
            ...
            {
                Debug.WriteLine("Module get error");
                TryGetModule = false;
                return false;
            }
        }
        ...
        public static int FindPattern(byte[] pattern, string mask, int moduleBase, int moduleSize, int skip = 0)
        {
            ...
            Debug.WriteLine($"Warning! {string.Join(" ", pattern.Select(x => x.ToString("X")))} {mask} is broken.");
        ...
        public class Allocator
        {
            ...
            {
                var address = VirtualAllocEx(Memory.pHandle, IntPtr.Zero, (IntPtr)4096,
            ...
            public void Free()
            {
                foreach (var key in AllocatedSize)
                    VirtualFreeEx(Memory.pHandle, key.Key, 4096, (int)0x1000 | (int)0x2000);
            }
        ...
        {
            //Debug.WriteLine("Update checked...");

            if (!Memory.OpenProcess("PointBlank"))
            {
                TryGetProcess = false;
                return false;
            }
            ...
            {
                Debug.WriteLine("Process handled...");
            }
            Thread.Sleep(100);
            ...
            {
                Debug.WriteLine("Module get succses...");
                TryGetModule = true;
            }
            return true;
        }
